...
class ApartmentPlacer:
    ...
    def place_apartments(self):
        ...
        debug(f"Free cells in rows 1 and 3: {len(free_cells)}")
        ...
        if self.base_percents.get('one_room', 0) > 0:
            for side in ['left', 'right']:
                cells = self._find_vertical_end_position(side)
                if cells:
                    area = self.apartment_configs['one_room_vertical_end']['area_m2']
                    if total_area + area <= self.max_square_apart:
                        ok, area = self.insert_apartment('one_room_vertical_end', cells, is_end=True, is_left=(side=='left'))
                        if ok:
                            placed.append({'type': 'one_room_vertical_end', 'cells': [c['id'] for c in cells], 'area': area, 'side': side})
                            total_area += area
                            debug(f"    Vertical 1-room ({side}), area: {area:.1f}")

        if self.base_percents.get('three_room', 0) > 0:
            for side in ['left', 'right']:
                cells = self._find_l_shape_end_position(side)
                if cells:
                    area = self.apartment_configs['three_room_end']['area_m2']
                    if total_area + area <= self.max_square_apart:
                        ok, area = self.insert_apartment('three_room_end', cells, is_end=True, is_left=(side=='left'))
                        if ok:
                            placed.append({'type': 'three_room_end', 'cells': [c['id'] for c in cells], 'area': area, 'side': side})
                            total_area += area
                            debug(f"    L-shaped 3-room ({side}), area: {area:.1f}")

        # 2. Расчёт количества квартир по процентам
        free_cells = self._get_free_cells_in_rows([1, 3])
        ...
        # 3. Заполнение рядовых ячеек
        debug("\n  3. FILLING REMAINING CELLS (rows 1 and 3)")
        ...
        # 4. Дозаполнение всех оставшихся ячеек студиями
        remaining = self._get_free_cells_in_rows([1, 3])
        if remaining:
            debug(f"\n  4. FILLING REMAINING {len(remaining)} CELLS WITH STUDIOS")
